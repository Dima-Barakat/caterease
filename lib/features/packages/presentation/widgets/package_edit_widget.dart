...
class PackageEditWidget extends StatefulWidget {
  final PackageDetail packageDetail;
  final dynamic cartItem;
  final bool isEditMode;

  const PackageEditWidget({
    Key? key, 
  ...
  @override
  State<PackageEditWidget> createState() => _PackageEditWidgetState();
}

class _PackageEditWidgetState extends State<PackageEditWidget> {
  ...
  void initState() {
    super.initState();
    if (widget.isEditMode && widget.cartItem != null) {
  ...
  void _initializeFromCartItem() {
    ...
    // Initialize extra persons
    extraPersons = cartItem.extraPersons ?? 0;
    
    // Initialize selected service types
    selectedServiceTypes = [];
    if (cartItem.serviceTypes != null) {
      for (var cartServiceType in cartItem.serviceTypes) {
        final matchingServiceType = widget.packageDetail.serviceType
            .where((st) => st.id == cartServiceType.id)
            .firstOrNull;
        if (matchingServiceType != null) {
          selectedServiceTypes.add(matchingServiceType);
        }
      }
    }
    
    // Initialize selected occasion types
    selectedOccasionTypes = [];
    if (cartItem.occasionTypeId != null) {
      final matchingOccasionType = widget.packageDetail.occasionTypes
          .where((ot) => ot.id == cartItem.occasionTypeId)
          .firstOrNull;
      if (matchingOccasionType != null) {
        selectedOccasionTypes = [matchingOccasionType];
      }
    }
    ...
    if (cartItem.extras != null) {
      for (var cartExtra in cartItem.extras) {
        final matchingExtra = widget.packageDetail.extras
            .where((ex) => ex.id == cartExtra.id)
            .firstOrNull;
        if (matchingExtra != null) {
          selectedExtras.add(matchingExtra);
        }
      }
    }
  }
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("الباكج: ${widget.packageDetail.name}"),
            const Divider(),
            Text(
                "الخدمات: ${selectedServiceTypes.map((e) => e.name).join(", ")}"),
            Text(
                "المناسبة: ${selectedOccasionTypes.map((e) => e.name).join(", ")}"),
            if (extraPersons > 0) Text("أشخاص إضافيين: $extraPersons"),
            if (selectedExtras.isNotEmpty)
              Text("الإضافات: ${selectedExtras.map((e) => e.name).join(", ")}"),
            if (widget.packageDetail.discount != null) ...[
              const SizedBox(height: 8),
              Text(
                  "الخصم: ${widget.packageDetail.discount!.value} - ${widget.packageDetail.discount!.description}",
                  style: TextStyle(
                      color: Colors.red, fontWeight: FontWeight.bold)),
            ],
            const SizedBox(height: 12),
        ...
        actions: [
          ...
          ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                final updateRequest = UpdateCartItemRequest(
                  cartItemId: widget.cartItem.cartItemId,
                  quantity: widget.cartItem.quantity ?? 1,
                  extraPersons: extraPersons,
                  occasionTypeId: selectedOccasionTypes.first.id,
                  serviceType: selectedServiceTypes
                      .map((st) => UpdateServiceType(id: st.id))
                      .toList(),
                  extras: selectedExtras
                      .map((ex) => UpdateExtra(extraId: ex.id, quantity: 1))
                      .toList(),
                );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.isEditMode ? "تعديل الباكج" : "تفاصيل الباكج"),
        centerTitle: true,
        elevation: 0,
      ),
        ...
        listener: (context, state) {
          if (state is CartLoading) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(widget.isEditMode ? "جاري التحديث..." : "جاري الإضافة للسلة...")),
            );
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Text(
                widget.isEditMode ? "تحديث الطلب" : "Place Order", 
